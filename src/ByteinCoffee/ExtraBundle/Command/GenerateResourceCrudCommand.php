...
use ByteinCoffee\ExtraBundle\Generator\ResourceCrudGenerator;

/**
 * Command gerador de telas e rotas para o crud de um resource
 *
 * @author Fábio Lemos Elizandro <fabio@elizandro.com.br>
 */
class GenerateResourceCrudCommand extends GenerateDoctrineCommand
{
    ...
    protected function configure()
    {
        $this
                ->setDefinition(array(
                    ...
                    new InputOption('bundle', '', InputOption::VALUE_REQUIRED, 'Bundle onde vai ser escrito todas as classes'),
...
EOT
                ...
                ->setName('byteincoffee:resource:crud')
                ->setAliases(array('byteincoffee:resource:generatecrud'))
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $bundleToWrite = $this->getContainer()->get('kernel')->getBundle($input->getOption('bundle'));
        ...
        $generator = $this->getGenerator($bundleToWrite);
        $generator->generate($bundleToWrite, $entity, $metadata, $format, $prefix, $withWrite, $forceOverwrite);
        ...
        if ('annotation' != $format) {
            $runner($this->updateRouting($dialog, $input, $output, $bundleToWrite, $format, $entity, $prefix));
        }
    ...
    protected function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln(array(
            '',
            'Você tem que informar qual o bundle onde será gerado o cadastro <comment>AcmeBlogBundle</comment>.',
            '',
        ));
        $bundleToWrite = $dialog->ask($output, $dialog->getQuestion('Bundle onde vai ser escrito as classes ?', $input->getOption('bundle')));
        $input->setOption('bundle', $bundleToWrite);
        ...
        $withWrite = $dialog->askConfirmation($output, $dialog->getQuestion('Do you want to generate the "write" actions', $withWrite ? 'yes' : 'y', '?'), $withWrite);
    ...
    {
        try {
            $this->getFormGenerator($bundle)->generate($bundle, $entity, $metadata);
    ...
    protected function updateRouting(DialogHelper $dialog, InputInterface $input, OutputInterface $output, BundleInterface $bundle, $format, $entity, $prefix)
    {
        ...
        $this->getContainer()->get('filesystem')->mkdir($bundle->getPath() . '/Resources/config/backend');
        $routing = new RoutingManipulator($bundle->getPath() . '/Resources/config/backend/routing.yml');
        try {
            $ret = $auto ? $routing->addResource($bundle->getName(), $format, '/' . $prefix, 'backend/' . strtolower(str_replace('\\', '_', $entity))) : false;
        ...
        if (!$ret) {
            ...
            return array(
                ...
                sprintf('  (%s).', $bundle->getPath() . '/Resources/config/backend/routing.yml'),
                '',
    ...
    {
        return new ResourceCrudGenerator($this->getContainer()->get('filesystem'));
    }
